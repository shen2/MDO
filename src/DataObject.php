...
abstract class DataObject extends \ArrayObject
{
	...
	{
		$select = new Select(static::$_db);
		$select->setTable(get_called_class());
	...
	{
		return (new Update(static::$_db))
			->table((static::$_schema ? static::$_schema . '.' : '') . static::$_name)
			->set($data)
			->whereClauses($where)
			->query();
	}
	...
	{
		return (new Delete(static::$_db))
			->from((static::$_schema ? static::$_schema . '.' : '') . static::$_name)
			->whereClauses($where)
			->query();
	}
	 ...
	 * @return Select
	 */
	protected static function _where(Select $select, $where)
	{
		$where = (array) $where;
